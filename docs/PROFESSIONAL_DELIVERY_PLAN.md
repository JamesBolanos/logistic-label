...
| Vercel | SvelteKit builds, per-branch preview deployments, the stable staging deployment when needed, production deployment, and deployment status |
| Neon | PostgreSQL hosting and isolated development, test, preview, staging, and production databases |
...
├── development branch              persistent synthetic development data
├── preview-base branch             clean synthetic baseline
├── preview/<git-branch> branches   ephemeral Vercel preview data
├── staging branch                  stable release/authentication verification
└── test/<run-id> branches           disposable automated-test data
```

| Environment | Code/deployment | Database | Data and lifecycle |
|---|---|---|---|
| Local development | Local short-lived branch and SvelteKit server | Non-production `development` branch | Synthetic; persistent but resettable |
...
| Pull-request preview | Vercel preview for the feature branch | Neon preview branch derived from `preview-base` | Synthetic; removed with the preview/Git branch |
...
Preview and test environments must never inherit or query production user data. Connect Vercel Preview to the non-production Neon project and Vercel Production only to the production Neon project. Scope credentials and environment variables accordingly.
...
2. Develop against the Neon non-production development branch and run the fast local quality command.
3. Open a GitHub pull request as the review, traceability, and evidence record.
4. Run GitHub Actions quality checks while Vercel creates the preview deployment.
5. Create the preview database branch, apply its committed Drizzle migrations, and seed only synthetic data.
6. Run Playwright and smoke checks against the preview URL; inspect the diff, preview, migration SQL, and logs.
...
9. Run the production smoke checklist, review errors, record the release, and remove the short-lived Git and database branches.

Use a persistent staging branch only for release-critical verification that needs a stable domain. Routine changes should move from an isolated pull-request preview directly to Production after checks pass.
...
- [ ] Create separate Neon production and non-production projects. Establish non-production `development`, `preview-base`, and `staging` branches with synthetic data only.
- [ ] Connect Vercel Preview exclusively to the non-production Neon project and Vercel Production exclusively to the production Neon project; verify the boundary with connection-denial tests where practical.
- [ ] Configure automatic Neon branches for Vercel previews, with naming, retention, and deletion tied to the Git/preview lifecycle.
- [ ] Configure a stable staging branch/domain for Google OAuth, real CAPTCHA-domain, callback, and cookie verification when those areas change.
...
- [ ] Make preview deployments apply committed Drizzle migrations to their preview branch before application verification; serialize migrations per branch.
